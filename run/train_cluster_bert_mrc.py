...
def args_parser():
    ...
    parser.add_argument("--num_clusters", type=int, default=10)#设置的是23
    ...
    parser.add_argument("--clus_niter", type=int, default=20)#设置的是60
...
#data_loader/mrc_data_loader.py 47行输出
#data_loader/mrc_data_loader.py 48行输出
#data_loader/mrc_data_loader.py 79行输出
...
def load_model(config, num_train_steps, label_list, pretrain=None):
    ...
    if pretrain:
        ...
        """
        model = BertMRCNER_CLUSTER(config)#model/bert_mrc_ner_cluster.py104行定义了
...
def eval_checkpoint(model_object, eval_dataloader, config, \
    ...
    model_object.eval()#因为是并行的，所以同前面一致也是调用的BertMRCNER_CLUSTER模型
    ...
    for input_ids, input_mask, segment_ids, start_pos, end_pos, span_pos, ner_cate in eval_dataloader:
        ...
        eval_loss += tmp_eval_loss.mean().item()#.mean()是求均值，item()取出单元素张量的元素值并返回该值，保持原元素类型不变
        mask_lst += input_mask
        eval_steps += 1
    ...
    #metric/mrc_ner_evaluate.py97 行中定义
    ...
    average_loss = round(eval_loss / eval_steps, 4)#eval_step=32因为设置的train&dev batch_size=32
...
def train_cluster(model, optimizer, train_dataloader, dev_dataloader, test_dataloader,
    ...
    for step, batch in enumerate(train_dataloader):#step是索引，batch是train_dataloader的数据
        batch = tuple(t.to(device) for t in batch)
        input_ids, input_mask, segment_ids, start_pos, end_pos, span_pos, ner_cate, cluster_pos = batch#batch里面有
        cluster_pos = cluster_pos.cpu().numpy()

        input_truth = []
     
        for i in cluster_pos:
            for k in range(i[0], i[1]):#从i[0]开始到i[1]结束
                input_truth.append(cluster_dict[k])
                #print("请输出cluster_dict：")
                #print(cluster_dict)#204596: 10
                #print("请输出cluster_pos：")
                #print(cluster_pos)#204596: 10
        input_truth = torch.tensor(input_truth, dtype=torch.long).to(device)#用于生成新的张量
        ...
        if (step + 1) % config.gradient_accumulation_steps == 0:
            optimizer.step()
            global_step += 1
        #因为287314/16=17958，所以输出的是17958
        if nb_tr_steps % config.checkpoint == 0 or nb_tr_steps==config.num_train_step:#checkpoiint＝2000所以在这里输出
            print("-*-" * 15)
            print("current instances is : ", nb_tr_steps)#2000-4000-6000-8000-10000-12000-14000-16000（因为一共17958）
            print("current training loss is : ")
            print(loss.item())#输出见model/bert_mrc_ner_cluster.py 320行损失中也加入了聚类的损失(特征分到聚类的损失与真实列表)
            # continue
            ...
            print("......" * 10)
            print("DEV: loss, acc, precision, recall, f1")# loss：model/bert_mrc_ner_cluster.py322行
            print(tmp_dev_loss, tmp_dev_acc, tmp_dev_prec, tmp_dev_rec, tmp_dev_f1)#acc,prec,rec,f1这四个数值在:metric/mrc_ner_evaluate.py 157行定义

            if tmp_dev_f1 > dev_best_f1 or tmp_dev_acc > dev_best_acc:#只要Dev阶段的f1>best_f1 或 dev阶段acc > best_acc
                dev_best_acc = tmp_dev_acc                            #就让best_acc & best_f1 = Dev阶段的 acc & f1
...
def compute_features(dataloader, model, N, config, device):
    ...
    for i, batch in enumerate(dataloader):
        ...
        # 测量经过的时间
        batch_time.update(time.time() - end)#当前时间-开始时间
        end = time.time()

        if i % 1000 == 0:#i=1000、2000、3000...  dataloader的长度是17958(287314分成train_batch_size=16，得到每份是17958)
            print('{0} / {1}\t'# 训练集每1000条数据输入到模型的训练时间
                  'Time: {batch_time.val:.3f} ({batch_time.avg:.3f})'#val将一组字符型数据的数字部分转换成相应的数值型数据
                  .format(i, len(dataloader), batch_time=batch_time))#avg平均时间
...
def main():
    ...
    deepcluster = clustering.__dict__[config.clustering](config.num_clusters)#clustering是k-means，num_clusters=23
    ...
    for epoch in range(int(config.num_train_epochs)):
        ...
        clustering_loss, cluster_var = deepcluster.cluster(features, config.view_number, config.cluster_layer, config.pca_dim,
                                              config.clus_niter, epoch, verbose=True)#是true
        train_cluster_loader = get_cluster_dataloader(config, train_input_ids, train_input_mask,
                                                      train_segment_ids, train_ner_cate,
                                                      train_start_pos,train_end_pos)

        cluster_dict = clustering.cluster_assign(deepcluster.images_lists)#model/clustering.py73行定义cluster_assign
        ...
        model, dev_best_acc, dev_best_f1, test_best_acc, test_best_f1 = train_cluster(model, optimizer,
                                                         ...
                                                         epoch)
    print("=&=" * 15)
    print("DEV: current best f1, acc")#dev_best_f1/acc都等于Dev阶段运行最大的f1/acc
