...
def compute_acc(pred_label, gold_label):
    dict_match = list(filter(lambda x: x[0] == x[1], zip(pred_label, gold_label)))#只过滤出预测标签与真实标签相等的列表
...
def update_label_lst(label_lst):
    """
    Label_lst是一个实体类别列表，例如: ["NS", "NT", "NM"]
    更新之后 ["B-NS", "E-NS", "S-NS"]
    """
    update_label_lst = [] 
...
def flat_transform_bmes_label(start_labels, end_labels, span_labels, ner_cate, threshold=0.5):
    bmes_labels = len(start_labels)*["O"]#现将所有位置都标上o
    ...
    for start_item in start_labels:
        bmes_labels[start_item] = "B-{}".format(ner_cate)#BMES：B表示这个字是词的开始Begin，M表示词的中间Middle，E表示词的结尾End，S表示单字成词,O代表others
...
def nested_transform_span_triple(start_labels, end_labels, span_labels, ner_cate, print_info=False, threshold=0.5):
    span_triple_lst = []
    #span_triple_lst中的元素是(ner_cate, start_index, end_index)
...
def flat_ner_performance(pred_start, pred_end, pred_span, \
    ...
    up_label_lst = update_label_lst(label_lst)#更新label_lst
    ...
    if dims == 1:
        ...
        # ner_cate: ["PER", "LOC", "ORG"]中的一个

        # 转换(开始，结束，跨度)标签到传统的BMESO标记方案
        # pred_bmes_label, gold_bmes_label: 列表如["B-PER", "M-PER", ]
        pred_bmes_label = flat_transform_bmes_label(pred_start, pred_end, pred_span, ner_cate, threshold=threshold)#给所有位置打"O""B""E""M""S"标签
        gold_bmes_label = flat_transform_bmes_label(gold_start, gold_end, gold_span, ner_cate, threshold=threshold)

        # 映射一个字符串列表 ["B-PER", "M-PER", ] to a list of label index [1, 2, 3]
        ...
        for pred_start_item, pred_end_item, pred_span_item, gold_start_item, gold_end_item, gold_span_item, ner_cate_item in zip(pred_start, pred_end, \
            ...
            #计算acc
            tmp_acc = compute_acc(pred_bmes_idx_lst, gold_bmes_idx_lst)#调用17行的算法计算预测id的列表，与真实id列表
